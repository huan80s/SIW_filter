...
//*********************************************************************************//
void scan_coef_Matrix_end1(complex** Coef, int num_total, int ** Coef_location, int sizeM, int sizeN)
{
	int i=0,j=0,m=0;
	...
	for ( i=(Nx_side); i<( 2*Nx_side ); i++ )//source node
	{
		...
		{
		case 0:
			boundary_node( j, i, 1, -Nx_side, Nx_side, Coef, Coef_location );
			break;
		case (Nx_side-1):
			boundary_node( j, i, 2, -Nx_side, Nx_side, Coef, Coef_location );
			break;
		case (N_d0-1):
			inner_4node( j, i, -Nx_side, -1, 0, Nx_side, 1, 1, k*k*16*dx*dx-4, 1, Coef, Coef_location );
			break;
		case N_d0:
		...
		case (N_d0+N_a-1):
			...
			break;
		case (N_d0+N_a):
			inner_4node( j, i, -Nx_side, 0, 1, Nx_side, 1, k*k*16*dx*dx-4, 1, 1, Coef, Coef_location );
			break;
		default:
			...
			{
				inner_5node( j, i, 4, -Nx_side, Nx_side, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
...
//****************************************************************************************!!!
	for ( i=( Nx_side*N_d1 ); i<( Nx_side*N_d1+Nx_side ); i++ )//middle right ,one line
	{
		...
		{
		case 0:
			boundary_node( j, i, 1, -Nx_side, Nx_side+Nx_1, Coef, Coef_location );
			break;
		case (Nx_side-1):
			boundary_node( j, i, 2, -Nx_side, Nx_2+Nx_1, Coef, Coef_location );
			break;
		case (N_d0-2):// 2 lei
			kind2_node( j, i, -Nx_side*2, -Nx_side, -1, 0, 1, Nx_side-N_d0+2, 
				B5, B4, B3, B1_1, B3, B2, Coef, Coef_location );
			break;
		case (N_d0-1):// 2 lei缺一点
			other_5_node( j, i, -2*Nx_side, -Nx_side, -1, 0, (Nx_side-N_d0+1+2), 
				B5, B4, B3, B1_1, B2, Coef, Coef_location );
			break;
		case N_d0:// 2 lei缺一点
			other_5_node( j, i, -2*Nx_side, -Nx_side, 0, 1, (Nx_side-N_d0+Nx_1/2-3), 
				B5, B4, B1_1, B3, B2, Coef, Coef_location );
			break;
		case (N_d0+1):// 2 lei
			kind2_node( j, i, -Nx_side*2, -Nx_side, -1, 0, 1, Nx_side-N_d0-1+Nx_1/2-1, 
				B5, B4, B3, B1_1, B3, B2, Coef, Coef_location );
			break;
		case (N_d0+N_a-2):// 2 lei
			kind2_node( j, i, -Nx_side*2, -Nx_side, -1, 0, 1, (Nx_side-N_d0-N_a+2+Nx_1/2), 
				B5, B4, B3, B1_1, B3, B2, Coef, Coef_location );
			break;
		case (N_d0+N_a-1):// 2 lei缺一点
			other_5_node( j, i, -2*Nx_side, -Nx_side, -1, 0, (Nx_side-N_d0-N_a+1+Nx_1/2+2), 
				B5, B4, B3, B1_1, B2, Coef, Coef_location );
			break;
		case (N_d0+N_a):// 2 lei缺一点
			other_5_node( j, i, -2*Nx_side, -Nx_side, 0, 1, (Nx_side-N_d0-N_a+Nx_1-3), 
				B5, B4, B1_1, B3, B2, Coef, Coef_location );
			break;
		case (N_d0+N_a+1):// 2 lei
			kind2_node( j, i, -Nx_side*2, -Nx_side, -1, 0, 1, (Nx_side-N_d0-N_a-1+Nx_1-1), 
				B5, B4, B3, B1_1, B3, B2, Coef, Coef_location );
			break;
		default:// 4dx
			if ( (i-Nx_side*N_d1) < (N_d0-2) )//up
			{
				inner_5node( j, i, 4, -Nx_side, Nx_side+Nx_1, Coef, Coef_location );
			} 
			else if ( (i-Nx_side*N_d1) > (N_d0+N_a+1) )//down
			{
				inner_5node( j, i, 4, -Nx_side, Nx_2+Nx_1, Coef, Coef_location );
			} 
			else//middle
			{
				inner_5node( j, i, 4, -Nx_side, Nx_side-N_d0-1+Nx_1+Node_1, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
...
//**********************************************************************************************
	for ( i=( Nx_side*N_d1+Nx_side ); i<( Nx_side*N_d1+Nx_side+Nx_1/2 ); i++ )//right1 Nx_1: 26 node part1
	{
		...
		{
		case 0:// 3 lei
			kind3_node( j, i, -(Nx_side-N_d0+2)-1, -(Nx_side-N_d0+2), 0, 1, Nx_1+N_d0-3, Nx_1+N_d0-1, 
				C4, C3_1, C1_1, C2_1, C4, C3_1, Coef, Coef_location );
			break;
		case (Nx_1/2-1)://node2: 3 lei
			kind3_node( j, i, -(Nx_side-N_d0-1+Nx_1/2-1), -(Nx_side-N_d0-1+Nx_1/2-1)+1, -1, 0, Nx_1+N_d0-1, Nx_1+N_d0-1+2, 
				C3_1, C4, C2_1, C1_1, C3_1, C4, Coef, Coef_location );
			break;
		case 1:// 1 lei
			kind1_node(j, i, -(Nx_side*2-N_d0+2+1), -(Nx_side*2-N_d0+2+1)+1, -(Nx_side-N_d0+2+1), -(Nx_side-N_d0+2), -1, 0, 1, Nx_1+N_d0-1, 
				       A2_1, A2_1, A1_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		case 2:// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_side-N_d0+1+2), Nx_1+N_d0-1, Coef, Coef_location );
			break;
		case 3:// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_side-N_d0+1+3)-Nx_side, -(Nx_side-N_d0+1+3), -1, 0, 1, 
				Nx_1+N_d0-1, A2_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		case (Nx_1/2-4):// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_side-N_d0+Nx_1/2-4)-Nx_side, -(Nx_side-N_d0+Nx_1/2-4), -1, 0, 1, 
				Nx_1+N_d0-1, A2_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		case (Nx_1/2-3):// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_side-N_d0+Nx_1/2-3), Nx_1+N_d0-1, Coef, Coef_location );
			break;
		case (Nx_1/2-2):// 1 lei
			kind1_node(j, i, -(Nx_side*2-N_d0+Nx_1/2-2), -(Nx_side*2-N_d0+Nx_1/2-2)+1, -(Nx_side-N_d0+Nx_1/2-2), -(Nx_side-N_d0+Nx_1/2-2)+1, 
				-1, 0, 1, Nx_1+N_d0-1, 
				A2_1, A2_1, A1_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		default:
			...
			{
				inner_4node( j, i, -1, 0, 1, Nx_1+N_d0-1, 1, k*k*4*dx*dx-4, 1, 1, Coef, Coef_location );
			}
			else// 1 lei que 4 dian
			{
				inner_4node(j, i, -1, 0, 1, Nx_1+N_d0-1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
	...
	for ( i=( Nx_side*N_d1+Nx_side+Nx_1/2 ); i<( Nx_side*N_d1+Nx_side+Nx_1 ); i++ )// Nx_1 :  26 node! part2
	{
		...
		{
		case 0:// 3 lei
			kind3_node( j, i, -(Nx_side-N_d0-N_a+2+Nx_1/2)-1, -(Nx_side-N_d0-N_a+2+Nx_1/2), 0, 1, Node_11-2, Node_11, 
				C4, C3_1, C1_1, C2_1, C4, C3_1, Coef, Coef_location );
			break;
		case (Nx_1/2-1):// 3 lei
			kind3_node( j, i, -(Nx_side-N_d0-N_a+Nx_1-2), -(Nx_side-N_d0-N_a+Nx_1-2)+1, -1, 0, Node_11, Node_11+2, 
				C3_1, C4, C2_1, C1_1, C3_1, C4, Coef, Coef_location );
			break;
		case 1:// 1 lei
			kind1_node(j, i, -(Nx_side*2-N_d0-N_a+2+Nx_1/2)-1, -(Nx_side*2-N_d0-N_a+2+Nx_1/2), -(Nx_side-N_d0-N_a+2+Nx_1/2)-1, -(Nx_side-N_d0-N_a+2+Nx_1/2), 
				-1, 0, 1, Node_11, 
				A2_1, A2_1, A1_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		case 2:// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_side-N_d0-N_a+1+Nx_1/2+2), Node_11, Coef, Coef_location );
			break;
		case 3:// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_side-N_d0-N_a+1+Nx_1/2+3)-Nx_side, -(Nx_side-N_d0-N_a+1+Nx_1/2+3), -1, 0, 1, 
				Node_11, A2_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		case (Nx_1/2-4):// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_side-N_d0-N_a+Nx_1-4)-Nx_side, -(Nx_side-N_d0-N_a+Nx_1-4), -1, 0, 1, Node_11, 
				A2_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		case (Nx_1/2-3):// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_side-N_d0-N_a+Nx_1-3), Node_11, Coef, Coef_location );
			break;
		case (Nx_1/2-2):// 1 lei
			kind1_node(j, i, -(Nx_side*2-N_d0-N_a+Nx_1-2), -(Nx_side*2-N_d0-N_a+Nx_1-2)+1, -(Nx_side-N_d0-N_a+Nx_1-2), -(Nx_side-N_d0-N_a+Nx_1-2)+1, 
				-1, 0, 1, Node_11, A2_1, A2_1, A1_1, A1_1, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			break;
		default:
			if ( ( i-Nx_side*N_d1-Nx_side-Nx_1/2 ) % 2 == 0 )
			{
				inner_4node( j, i, -1, 0, 1, Node_11, 1, k*k*4*dx*dx-4, 1, 1, Coef, Coef_location );
			} 
			else//1 lei
			{
				inner_4node(j, i, -1, 0, 1, Node_11, A3_1, A5_1, A3_1, A4_1, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
...
//***************************************************************************************
	for ( i=( Nx_side*N_d1+Nx_side+Nx_1 ); i<( Nx_side*N_d1+Nx_side+Nx_1+Nx_2 ); i++ )//right 2 Nx_2 :  77 node! 
	{
		...
		{
		case 0:
			boundary_node( j, i, 1, -Nx_side-Nx_1, Nx_2+Nx_3, Coef, Coef_location );
			break;
		case (Nx_2-1):
			boundary_node( j, i, 2, -Nx_1-Nx_2, Nx_2+Nx_3, Coef, Coef_location );
			break;
		case (N_d0-3)://2 lei shang
			kind2_node( j, i, -Nx_side-Nx_1, -2, -1, 0, 1, Nx_2+Nx_3, 
				B3, B5, B4, B1_1, B2, B3, Coef, Coef_location );
			break;
		case (N_d0-2)://3 lei shang
			kind3_node( j, i, -Nx_side-Nx_1-1, -Nx_side-Nx_1, -1, 0, 1, Nx_2-N_d0+2, 
				C4, C4, C3_1, C1_1, C3_1, C2_1, Coef, Coef_location );
			break;
		case Node_1://3 lei xia
			kind3_node( j, i, -(Nx_side-N_d0-1+Nx_1+Node_1), -(Nx_side-N_d0-1+Nx_1+Node_1)+1, -1, 0, 1, Nx_2-N_d0+2, 
				C4, C4, C3_1, C1_1, C3_1, C2_1, Coef, Coef_location );
			break;
		case (Node_1+1)://2 lei xia
			kind2_node( j, i, -(Nx_side-N_d0-1+Nx_1+Node_1), -1, 0, 1, 2, Nx_2+Nx_3, 
				B3, B2, B1_1, B4, B5, B3, Coef, Coef_location );
			break;
		case (Node_1+N_a-4)://2 lei shang
			kind2_node( j, i, -(Nx_side-N_d0-1+Nx_1+Node_1), -2, -1, 0, 1, Nx_2+Nx_3, 
				B3, B5, B4, B1_1, B2, B3, Coef, Coef_location );
			break;
		case (Node_1+N_a-3)://3 lei shang
			kind3_node( j, i, -(Nx_side-N_d0-1+Nx_1+Node_1)-1, -(Nx_side-N_d0-1+Nx_1+Node_1), -1, 0, 1, Nx_2-Node_11+Nx_3-1, 
				C4, C4, C3_1, C1_1, C3_1, C2_1, Coef, Coef_location );
			break;
		case Node_11://3 lei xia
			kind3_node( j, i, -Nx_1-Nx_2, -Nx_1-Nx_2+1, -1, 0, 1, Nx_2-Node_11+Nx_3-1, 
				C4, C4, C3_1, C1_1, C3_1, C2_1, Coef, Coef_location );
			break;
		case (Node_11+1)://2 lei xia
			kind2_node( j, i, -Nx_1-Nx_2, -1, 0, 1, 2, Nx_2+Nx_3, 
				B3, B2, B1_1, B4, B5, B3, Coef, Coef_location );
			break;
		default:
			if ( (i-Nx_side*N_d1-Nx_1-Nx_side) < (N_d0-3) )// 4dx
			{
				inner_5node( j, i, 4, -Nx_side-Nx_1, Nx_2+Nx_3, Coef, Coef_location );
			} 
			else if ( ( (i-Nx_side*N_d1-Nx_1-Nx_side) >= (N_d0-1) ) && ( (i-Nx_side*N_d1-Nx_1-Nx_side) < Node_1 ) )// 2dx!
			{
				inner_5node( j, i, 2, -(Nx_1+N_d0-1), Nx_2-N_d0+2, Coef, Coef_location );
			} 
			else if ( ( (i-Nx_side*N_d1-Nx_1-Nx_side) > (Node_1+1) ) && ( (i-Nx_side*N_d1-Nx_1-Nx_side) < (Node_1+N_a-4) ) )// 4dx!
			{
				inner_5node( j, i, 4, -(Nx_side-N_d0-1+Nx_1+Node_1), Nx_2+Nx_3, Coef, Coef_location );
			} 
			else if ( ( (i-Nx_side*N_d1-Nx_1-Nx_side) >= (Node_1+N_a-2) ) && ( (i-Nx_side*N_d1-Nx_1-Nx_side) < Node_11 ) )// 2dx!
			{
				inner_5node( j, i, 2, -Node_11, Nx_2-Node_11+Nx_3-1, Coef, Coef_location );
			}
			else// 4dx!
			{
				inner_5node( j, i, 4, -Nx_1-Nx_2, Nx_2+Nx_3, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
...
//***************************************************************************************
	for ( i=(num_total-Nx_2); i<num_total; i++ )//right 4 输入波导/1区域虚拟边条，重叠区为 2*Nx_2+Nx_3
	{
		if ( (i-num_total+Nx_2) < (N_d0-2) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;//11
			Coef[j][0].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][0]=i-Nx_2-Nx_3;
			b[j].real=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real-kz*4*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image
				     -Ey[n][0*(Nx_2+Nx_3)+i-num_total+Nx_2].real;
			b[j].image=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image+kz*4*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real
				     -Ey[n][0*(Nx_2+Nx_3)+i-num_total+Nx_2].image;
		} 
		else if ( ( (i-num_total+Nx_2) >= (N_d0-2) ) && ( (i-num_total+Nx_2) <= Node_1 ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;//22
			Coef[j][0].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][0]=i-(Nx_3+N_d0-2);
			b[j].real=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real-kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image
			    	 -Ey[n][Nx_2+i-num_total+Nx_2-N_d0+2].real;
			b[j].image=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image+kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real
			    	 -Ey[n][Nx_2+i-num_total+Nx_2-N_d0+2].image;
		} 
		else if ( ( (i-num_total+Nx_2) >= (Node_1+1) ) && ( (i-num_total+Nx_2) <= (Node_1+N_a-4) ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;//33
			Coef[j][0].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][0]=i-Nx_2-Nx_3;
			b[j].real=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real-kz*4*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image
		...
		else if ( ( (i-num_total+Nx_2) >= (Node_1+N_a-3) ) && ( (i-num_total+Nx_2) <= Node_11 ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;//44
			Coef[j][0].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][0]=i-(Node_11+1);
			b[j].real=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real-kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image
				     -Ey[n][Nx_2+Nx_3/2+i-num_total+Nx_2-Node_1-N_a+3].real;
			b[j].image=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image+kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real
				     -Ey[n][Nx_2+Nx_3/2+i-num_total+Nx_2-Node_1-N_a+3].image;
		} 
		else
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;//55
			Coef[j][0].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][0]=i-Nx_2-Nx_3;
			b[j].real=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real-kz*4*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image
	...
	cout<<"port1区域节点数："<<j<<endl;
};
//*********************************************************************************//
//输出端口区域2参数扫描：n代表重叠区域号，Ey[n][M]代表每次迭代所求的解
//*********************************************************************************//
void scan_coef_Matrix_end2( int n, complex** Ey, complex** Coef, complex* b, int num_total, int** Coef_location )
{
	int i=0,j=0,m=0;
    ...
    //参数扫描
	j=0;
	for ( i=0; i<Nx_2; i++ )//left 4 输出波导/22区域虚拟边条，n=10
	{
		if ( i < (N_d0-2) )//1
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;
			Coef[j][4].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2+Nx_3;
			b[j].real=Ey[n][i].real-kz*4*dx*Ey[n][i].image-Ey[n][Nx_2+Nx_3+i].real;
			b[j].image=Ey[n][i].image+kz*4*dx*Ey[n][i].real-Ey[n][Nx_2+Nx_3+i].image;
		}
		else if ( ( i >= (N_d0-2) ) && ( i <= Node_1 ) )//2
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;
			Coef[j][4].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2-N_d0+2;
			b[j].real=Ey[n][i].real-kz*2*dx*Ey[n][i].image-Ey[n][Nx_2+i-N_d0+2].real;
			b[j].image=Ey[n][i].image+kz*2*dx*Ey[n][i].real-Ey[n][Nx_2+i-N_d0+2].image;
		} 
		else if ( ( i > Node_1 ) && ( i <= (Node_1+N_a-4) ) )//3
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;
			Coef[j][4].real=-1;
		...
		else if ( ( i >= (Node_1+N_a-3) ) && ( i <= Node_11 ) )//4
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;
			Coef[j][4].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2-Node_11+Nx_3-1;
			b[j].real=Ey[n][i].real-kz*2*dx*Ey[n][i].image-Ey[n][Nx_2+Nx_3/2+i-Node_1-N_a+3].real;
			b[j].image=Ey[n][i].image+kz*2*dx*Ey[n][i].real-Ey[n][Nx_2+Nx_3/2+i-Node_1-N_a+3].image;
		} 
		else
		{
			Coef[j][2].real=1;
...
//***************************************************************************************
	for ( i=( Nx_2+Nx_3 ); i<( Nx_2*2+Nx_3 ); i++ )//left 2 Nx_2:  77 node! 
	{
		...
		{
		case 0:
			boundary_node( j, i, 1, -Nx_2-Nx_3, Nx_2+Nx_1, Coef, Coef_location );
			break;
		case (Nx_2-1):
			boundary_node( j, i, 2, -Nx_3-Nx_2, Nx_1+Nx_side, Coef, Coef_location );
			break;
		case (N_d0-3)://2 lei shang
			kind2_node( j, i, -Nx_2-Nx_3, -2, -1, 0, 1, Nx_2+Nx_1, 
				B3, B5, B4, B1_1, B2, B3, Coef, Coef_location );
			break;
		case (N_d0-2)://3 lei shang
			kind3_node( j, i, -(Nx_3+N_d0-2), -1, 0, 1, Nx_2+Nx_1-1, Nx_2+Nx_1, 
				C2_1, C3_1, C1_1, C3_1, C4, C4, Coef, Coef_location );
			break;
		case Node_1://3 lei xia
			kind3_node( j, i, -(Nx_3+N_d0-2), -1, 0, 1, Nx_2-Node_1+Nx_1+N_d0+1, Nx_2-Node_1+Nx_1+N_d0+1+1, 
				C2_1, C3_1, C1_1, C3_1, C4, C4, Coef, Coef_location );
			break;
		case (Node_1+1)://2 lei xia
			kind2_node( j, i, -(Nx_2+Nx_3), -1, 0, 1, 2, Nx_2-Node_1+Nx_1+N_d0+1, 
				B3, B2, B1_1, B4, B5, B3, Coef, Coef_location );
			break;
		case (Node_1+N_a-4)://2 lei shang
			kind2_node( j, i, -(Nx_2+Nx_3), -2, -1, 0, 1, Nx_2-Node_1+Nx_1+N_d0+1, 
				B3, B5, B4, B1_1, B2, B3, Coef, Coef_location );
			break;
		case (Node_1+N_a-3)://3 lei shang
			kind3_node( j, i, -(Node_11+1), -1, 0, 1, Nx_2-Node_1+Nx_1+N_d0+1-1, Nx_2-Node_1+Nx_1+N_d0+1, 
				C2_1, C3_1, C1_1, C3_1, C4, C4, Coef, Coef_location );
			break;
		case Node_11://3 lei xia
			kind3_node( j, i, -(Node_11+1), -1, 0, 1, Nx_side+Nx_1, Nx_side+Nx_1+1, 
				C2_1, C3_1, C1_1, C3_1, C4, C4, Coef, Coef_location );
			break;
		case (Node_11+1)://2 lei xia
			kind2_node( j, i, -Nx_3-Nx_2, -1, 0, 1, 2, Nx_1+Nx_side, 
				B3, B2, B1_1, B4, B5, B3, Coef, Coef_location );
			break;
		default:
			if ( (i-Nx_2-Nx_3) < (N_d0-3) )// 2dx!
			{
				inner_5node( j, i, 4, -Nx_3-Nx_2, Nx_2+Nx_1, Coef, Coef_location );
			} 
			else if ( ( (i-Nx_2-Nx_3) >= (N_d0-1) ) && ( (i-Nx_2-Nx_3) < Node_1 ) )
			{
				inner_5node( j, i, 2, -(Nx_3+N_d0-2), Nx_2-N_d0+1, Coef, Coef_location );
			} 
			else if ( ( (i-Nx_2-Nx_3) > (Node_1+1) ) && ( (i-Nx_2-Nx_3) < (Node_1+N_a-4) ) )// 2dx!
			{
				inner_5node( j, i, 4, -(Nx_2+Nx_3), Nx_2-Node_1+Nx_1+N_d0+1, Coef, Coef_location );
			} 
			else if ( ( (i-Nx_2-Nx_3) >= (Node_1+N_a-2) ) && ( (i-Nx_2-Nx_3) < Node_11 ) )
			{
				inner_5node( j, i, 2, -(Node_11+1), Nx_1+Nx_2-Node_11, Coef, Coef_location );
			}
			else// 2dx!
			{
				inner_5node( j, i, 4, -Nx_3-Nx_2, Nx_side+Nx_1, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
...
//******************************************************************************************
	for ( i=( Nx_2*2+Nx_3 ); i<( Nx_2*2+Nx_3+Nx_1/2 ); i++ )// left 1 Nx_1: 26 node! part1
	{
		...
		{
		case 0:// 3 lei
			kind3_node( j, i, -(Nx_2-N_d0+1)-2, -(Nx_2-N_d0+1), 0, 1, Nx_1+N_d0-3, Nx_1+N_d0-2, 
				C4, C3_1, C1_1, C2_1, C4, C3_1, Coef, Coef_location );
			break;
		case (Nx_1/2-1)://node2: 3 lei
			kind3_node( j, i, -(Nx_2-N_d0+1), -(Nx_2-N_d0+1)+2, -1, 0, Nx_1/2+1+N_d0+1, Nx_1/2+1+N_d0+2, 
				C3_1, C4, C2_1, C1_1, C3_1, C4, Coef, Coef_location );
			break;
		case 1:// 1 lei
			kind1_node(j, i, -(Nx_2-N_d0+1), -1, 0, 1, (Nx_1-1+N_d0-2), (Nx_1-1+N_d0-2)+1, (Nx_1-1+N_d0-2)+Nx_side, (Nx_1-1+N_d0-2)+1+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A1_1, A2_1, A2_1, Coef, Coef_location );
			break;
		case 2:// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_2-N_d0+1), Nx_1-2+N_d0-1, Coef, Coef_location );
			break;
		case 3:// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_2-N_d0+1), -1, 0, 1, Nx_1-3+N_d0-1, Nx_1-3+N_d0-1+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A2_1, Coef, Coef_location );
			break;
		case (Nx_1/2-4):// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_2-N_d0+1), -1, 0, 1, N_d0+Nx_1/2+4, N_d0+Nx_1/2+4+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A2_1, Coef, Coef_location );
			break;
		case (Nx_1/2-3):// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_2-N_d0+1), Nx_1/2+3+N_d0, Coef, Coef_location );
			break;
		case (Nx_1/2-2):// 1 lei
			kind1_node(j, i, -(Nx_2-N_d0+1), -1, 0, 1, (Nx_1/2+2+N_d0), (Nx_1/2+2+N_d0)+1, (Nx_1/2+2+N_d0)+Nx_side, (Nx_1/2+2+N_d0)+1+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A1_1, A2_1, A2_1, Coef, Coef_location );
			break;
		default:
			if ( ( i-Nx_2*2-Nx_3 ) % 2 == 0 )
			{
				inner_4node( j, i, -(Nx_2-N_d0+1), -1, 0, 1, 1, 1, k*k*4*dx*dx-4, 1, Coef, Coef_location );
			}
			else//1 lei
			{
				inner_4node(j, i, -(Nx_2-N_d0+1), -1, 0, 1, A4_1, A3_1, A5_1, A3_1, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
	...
	for ( i=( Nx_2*2+Nx_3+Nx_1/2 ); i<( Nx_2*2+Nx_3+Nx_1 ); i++ )//left 1 Nx_1: 26 node! part2
	{
		...
		{
		case 0:// 3 lei
			kind3_node( j, i, -(Nx_2-Node_11+Nx_1)-2, -(Nx_2-Node_11+Nx_1), 0, 1, Nx_1/2+N_d0+N_a-3, Nx_1/2+N_d0+N_a-2, 
				C4, C3_1, C1_1, C2_1, C4, C3_1, Coef, Coef_location );
			break;
		case (Nx_1/2-1)://node2: 3 lei
			kind3_node( j, i, -(Nx_2-Node_11+Nx_1), -(Nx_2-Node_11+Nx_1)+2, -1, 0, N_d0+N_a+1+1, N_d0+N_a+2+1, 
				C3_1, C4, C2_1, C1_1, C3_1, C4, Coef, Coef_location );
			break;
		case 1:// 1 lei
			kind1_node(j, i, -(Nx_2-Node_11+Nx_1), -1, 0, 1, (Nx_1/2-1+N_d0+N_a-2), (Nx_1/2-1+N_d0+N_a-2)+1, 
				(Nx_1/2-1+N_d0+N_a-2)+Nx_side, (Nx_1/2-1+N_d0+N_a-2)+1+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A1_1, A2_1, A2_1, Coef, Coef_location );
			break;
		case 2:// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_2-Node_11+Nx_1), Nx_1/2-2+N_d0+N_a-1, Coef, Coef_location );
			break;
		case 3:// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_2-Node_11+Nx_1), -1, 0, 1, Nx_1/2-3+N_d0+N_a-1, Nx_1/2-3+N_d0+N_a-1+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A2_1, Coef, Coef_location );
			break;
		case (Nx_1/2-4):// 1 lei que 2 dian
			kind2_node(j, i, -(Nx_2-Node_11+Nx_1), -1, 0, 1, N_d0+N_a+4, N_d0+N_a+4+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A2_1, Coef, Coef_location );
			break;
		case (Nx_1/2-3):// 5 node 2dx
			inner_5node( j, i, 2, -(Nx_2-Node_11+Nx_1), 3+N_d0+N_a, Coef, Coef_location );
			break;
		case (Nx_1/2-2):// 1 lei
			kind1_node(j, i, -(Nx_2-Node_11+Nx_1), -1, 0, 1, (2+N_d0+N_a), (2+N_d0+N_a)+1, (2+N_d0+N_a)+Nx_side, (2+N_d0+N_a)+1+Nx_side, 
				A4_1, A3_1, A5_1, A3_1, A1_1, A1_1, A2_1, A2_1, Coef, Coef_location );
			break;
		default:
			if ( ( i-Nx_2*2-Nx_3-Nx_1/2 ) % 2 == 0 )// 2dx
			{
				inner_4node( j, i, -(Nx_2-Node_11+Nx_1), -1, 0, 1, 1, 1, k*k*4*dx*dx-4, 1, Coef, Coef_location );
			}
			else//1 lei
			{
				inner_4node(j, i, -(Nx_2-Node_11+Nx_1), -1, 0, 1, A4_1, A3_1, A5_1, A3_1, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
...
//******************************************************************************************!!!!!
	for ( i=( Nx_2*2+Nx_3+Nx_1 ); i<(num_total-Nx_side*N_d1); i++ )//middle left ,one line
	{
		...
		{
		case 0:
			boundary_node( j, i, 1, -Nx_2-Nx_1, Nx_side, Coef, Coef_location );
			break;
		case (Nx_side-1):
			boundary_node( j, i, 2, -Nx_1-Nx_side, Nx_side, Coef, Coef_location );
			break;
		case (N_d0-2):// 2 lei
			kind2_node( j, i, -(Nx_1+N_d0-2), -1, 0, 1, Nx_side, Nx_side*2, B2, B3, B1_1, B3, B4, B5, Coef, Coef_location );
			break;
		case (N_d0-1):// 2 lei缺一点
			other_5_node( j, i, -(Nx_1-2+N_d0-1), -1, 0, Nx_side, 2*Nx_side, B2, B3, B1_1, B4, B5, Coef, Coef_location );
			break;
		case N_d0:// 2 lei缺一点
			other_5_node( j, i, -(Nx_1/2+3+N_d0), 0, 1, Nx_side, 2*Nx_side, B2, B1_1, B3, B4, B5, Coef, Coef_location );
			break;
		case (N_d0+1):// 2 lei
			kind2_node( j, i, -(Nx_1/2+1+N_d0+1), -1, 0, 1, Nx_side, Nx_side*2, B2, B3, B1_1, B3, B4, B5, Coef, Coef_location );
			break;
		case (N_d0+N_a-2):// 2 lei
			kind2_node( j, i, -(Nx_1/2+N_a+N_d0-2), -1, 0, 1, Nx_side, Nx_side*2, B2, B3, B1_1, B3, B4, B5, Coef, Coef_location );
			break;
		case (N_d0+N_a-1):// 2 lei缺一点
			other_5_node( j, i, -(Nx_1/2-2+N_d0+N_a-1), -1, 0, Nx_side, 2*Nx_side, B2, B3, B1_1, B4, B5, Coef, Coef_location );
			break;
		case (N_d0+N_a):// 2 lei缺一点
			other_5_node( j, i, -(N_d0+N_a+3), 0, 1, Nx_side, 2*Nx_side, B2, B1_1, B3, B4, B5, Coef, Coef_location );
			break;
		case (N_d0+N_a+1):// 2 lei
			kind2_node( j, i, -(1+N_a+N_d0+1), -1, 0, 1, Nx_side, Nx_side*2, B2, B3, B1_1, B3, B4, B5, Coef, Coef_location );
			break;
		default:
			if ( (i-Nx_2*2-Nx_3-Nx_1) < (N_d0-2) )//up
			{
				inner_5node( j, i, 4, -Nx_2-Nx_1, Nx_side, Coef, Coef_location );
			} 
			else if ( (i-Nx_2*2-Nx_3-Nx_1) > (N_d0+N_a+1) )//down
			{
				inner_5node( j, i, 4, -Nx_1-Nx_side, Nx_side, Coef, Coef_location );
			} 
			else//middle
			{
				inner_5node( j, i, 4, -(Nx_2-Node_1+Nx_1+N_d0+1), Nx_side, Coef, Coef_location );
			}
			break;
		}
		j=j+1;
...
//******************************************************************************************
	for ( i=(num_total-Nx_side); i<num_total; i++ )//截断右边界 1阶mur吸收边界条件
	{
		...
		case (Nx_side-1):
			...
			Coef_location[j][0]=i-Nx_side-1;
			Coef_location[j][1]=i-Nx_side;
			Coef_location[j][2]=i-1;
			Coef_location[j][3]=i;
			break;
		case (N_d0-1):
			Coef[j][2].real=(2-2*k*k*16*dx*dx)*cos(k*4*dx)-2;
	...
	cout<<"port2区域节点数："<<j<<endl;
}
//*********************************************************************************//
//中间圆孔区域扫参（共20个圆孔）：n代表重叠区域号，Ey[n][M]代表每次迭代所求的解
//*********************************************************************************//
void scan_coef_Matrix_middle( int n, complex** Ey, complex** Coef, complex* b, int num_total, int** Coef_location )
{
	int i=0,j=0,m=0;
	for( j=0; j<num_total; j++ )         //系数矩阵初始置零
	{
    ...
    //系数矩阵扫描
//*********************************************************************
	for ( i=0; i<Nx_2; i++ )//left num 1 输出波导/22区域虚拟边条，n=10
	{
		if ( i < (N_d0-2) )//1
		{
			Coef[j][2].real=1;
			...
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2+Nx_3;
			b[j].real=Ey[n-1][i].real-kz*4*dx*Ey[n-1][i].image-Ey[n-1][Nx_2+Nx_3+i].real;
			b[j].image=Ey[n-1][i].image+kz*4*dx*Ey[n-1][i].real-Ey[n-1][Nx_2+Nx_3+i].image;
		}
		else if ( ( i >= (N_d0-2) ) && ( i <= Node_1 ) )//2
		{
			Coef[j][2].real=1;
			...
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2-N_d0+2;
			b[j].real=Ey[n-1][i].real-kz*2*dx*Ey[n-1][i].image-Ey[n-1][Nx_2+i-N_d0+2].real;
			b[j].image=Ey[n-1][i].image+kz*2*dx*Ey[n-1][i].real-Ey[n-1][Nx_2+i-N_d0+2].image;
		} 
		else if ( ( i > Node_1 ) && ( i <= (Node_1+N_a-4) ) )//3
		{
			Coef[j][2].real=1;
		...
		else if ( ( i >= (Node_1+N_a-3) ) && ( i <= Node_11 ) )//4
		{
			...
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2-Node_11+Nx_3-1;
			b[j].real=Ey[n-1][i].real-kz*2*dx*Ey[n-1][i].image-Ey[n-1][Nx_2+Nx_3/2+i-Node_1-N_a+3].real;
			b[j].image=Ey[n-1][i].image+kz*2*dx*Ey[n-1][i].real-Ey[n-1][Nx_2+Nx_3/2+i-Node_1-N_a+3].image;
		} 
		else
		{
			Coef[j][2].real=1;
...
//******************************************************************************************
	for ( i=(num_total-Nx_2); i<num_total; i++ )//right 2 输入波导/1区域虚拟边条，重叠区为 2*Nx_2+Nx_3
	{
		if ( (i-num_total+Nx_2) < (N_d0-2) )
		{
			Coef[j][2].real=1;
...
//*********************************************************************************//
void scan_coef_Matrix_post1( int n, complex** Ey, complex** Coef, complex* b, int num_total, int** Coef_location )
{
	int i=0,j=0,m=0;
	for( j=0; j<num_total; j++ )         //系数矩阵初始置零
	{
...
//*********************************************************************
	for ( i=0; i<Nx_2; i++ )//left num 1 输出波导/22区域虚拟边条，n=10
	{
		...
		else if ( ( i >= (N_d0-2) ) && ( i <= Node_1_p1 ) )//2
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;
			Coef[j][4].real=-1;
		...
		else if ( ( i > Node_1_p1 ) && ( i <= (Node_1_p1+N_a-4) ) )//3
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;
			Coef[j][4].real=-1;
		...
		else if ( ( i >= (Node_1_p1+N_a-3) ) && ( i <= Node_11_p1 ) )//4
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;
			Coef[j][4].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2-Node_11_p1+Nx_3-1;
			b[j].real=Ey[n-1][i].real-kz*2*dx*Ey[n-1][i].image-Ey[n-1][Nx_2+Nx_3/2+i-Node_1_p1-N_a+3].real;
			b[j].image=Ey[n-1][i].image+kz*2*dx*Ey[n-1][i].real-Ey[n-1][Nx_2+Nx_3/2+i-Node_1_p1-N_a+3].image;
		} 
		else
		{
			Coef[j][2].real=1;
...
//******************************************************************************************
	for ( i=(num_total-Nx_2); i<num_total; i++ )//right 2 输入波导/1区域虚拟边条，重叠区为 2*Nx_2+Nx_3
	{
		...
		else if ( ( (i-num_total+Nx_2) >= (N_d0-2) ) && ( (i-num_total+Nx_2) <= Node_1_p1 ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;//22
			Coef[j][0].real=-1;
		...
		else if ( ( (i-num_total+Nx_2) >= (Node_1_p1+1) ) && ( (i-num_total+Nx_2) <= (Node_1_p1+N_a-4) ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;//33
			Coef[j][0].real=-1;
		...
		else if ( ( (i-num_total+Nx_2) >= (Node_1_p1+N_a-3) ) && ( (i-num_total+Nx_2) <= Node_11_p1 ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;//44
			Coef[j][0].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][0]=i-(Node_11_p1+1);
			b[j].real=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real-kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image
				-Ey[n][Nx_2+Nx_3/2+i-num_total+Nx_2-Node_1_p1-N_a+3].real;
			b[j].image=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image+kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real
				-Ey[n][Nx_2+Nx_3/2+i-num_total+Nx_2-Node_1_p1-N_a+3].image;
		} 
		else
		{
			Coef[j][2].real=1;
...
//*********************************************************************************//
void scan_coef_Matrix_post2( int n, complex** Ey, complex** Coef, complex* b, int num_total, int** Coef_location )
{
	int i=0,j=0,m=0;
	for( j=0; j<num_total; j++ )         //系数矩阵初始置零
	{
...
//*********************************************************************
	for ( i=0; i<Nx_2; i++ )//left num 1 输出波导/22区域虚拟边条，n=10
	{
		...
		else if ( ( i >= (N_d0-2) ) && ( i <= Node_1_p2 ) )//2
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;
			Coef[j][4].real=-1;
		...
		else if ( ( i > Node_1_p2 ) && ( i <= (Node_1_p2+N_a-4) ) )//3
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;
			Coef[j][4].real=-1;
		...
		else if ( ( i >= (Node_1_p2+N_a-3) ) && ( i <= Node_11_p2 ) )//4
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;
			Coef[j][4].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][4]=i+Nx_2-Node_11_p2+Nx_3-1;
			b[j].real=Ey[n-1][i].real-kz*2*dx*Ey[n-1][i].image-Ey[n-1][Nx_2+Nx_3/2+i-Node_1_p2-N_a+3].real;
			b[j].image=Ey[n-1][i].image+kz*2*dx*Ey[n-1][i].real-Ey[n-1][Nx_2+Nx_3/2+i-Node_1_p2-N_a+3].image;
		} 
		else
		{
			Coef[j][2].real=1;
...
//******************************************************************************************
	for ( i=(num_total-Nx_2); i<num_total; i++ )//right 2 输入波导/1区域虚拟边条，重叠区为 2*Nx_2+Nx_3
	{
		...
		else if ( ( (i-num_total+Nx_2) >= (N_d0-2) ) && ( (i-num_total+Nx_2) <= Node_1_p2 ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;//22
			Coef[j][0].real=-1;
		...
		else if ( ( (i-num_total+Nx_2) >= (Node_1_p2+1) ) && ( (i-num_total+Nx_2) <= (Node_1_p2+N_a-4) ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*4*dx;//33
			Coef[j][0].real=-1;
		...
		else if ( ( (i-num_total+Nx_2) >= (Node_1_p2+N_a-3) ) && ( (i-num_total+Nx_2) <= Node_11_p2 ) )
		{
			Coef[j][2].real=1;
			Coef[j][2].image=kz*2*dx;//44
			Coef[j][0].real=-1;
			Coef_location[j][2]=i;
			Coef_location[j][0]=i-(Node_11_p2+1);
			b[j].real=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real-kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image
				-Ey[n][Nx_2+Nx_3/2+i-num_total+Nx_2-Node_1_p2-N_a+3].real;
			b[j].image=Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].image+kz*2*dx*Ey[n][1*(Nx_2+Nx_3)+i-num_total+Nx_2].real
				-Ey[n][Nx_2+Nx_3/2+i-num_total+Nx_2-Node_1_p2-N_a+3].image;
		} 
		else
		{
			Coef[j][2].real=1;
...
void main()// DDM-10 region
{
	...
	const int extra_nodes1 = Num2 + Num3 + Num3 + Num4 + Num5;	//有影响的节点数
	...
	int M1_post2 = M_post2 + 2 * extra_nodes2;
	...
	//获取系数矩阵 未完成！！！！！！！
	scan_coef_Matrix_end1(A0, M_end, b0, M0, N0);										//end
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_middle(A1_post1, M_post1, b1_post1, M1_post1, N1_post1);			//post1
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_middle(A1_post2, M_post2, b1_post2, M1_post2, N1_post2);			//post2
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_middle(A1_post1, M_post1, b1_post1, M1_post1, N1_post1);			//post1
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_middle(A1, M_middle, b1, M1, N1);
	scan_coef_Matrix_end2(A2, M_end, b2, M2, N2);										//end
	...
	while (bnorm > eps)
	{
		//1
		for (int i = 0; i != M0; i++)
			B0[i] = Binc[i + startpos[0]];
		cgmethod(A0, b0, B0, X0, M0, N0, N_matrix);
		if (times == 0)
			sinta = 1;
		else
			sinta = sinta1;
		...
		getB(Binc, A0, b0, X0, M0, N0, startpos[0]);

		////第一组
		//for (int i = 0; i != M1; i++)
		//	B1[i] = Binc[i + startpos[1]];
		//cgmethod(A1_0, b1_0, B1, X1, M1, N1, N_matrix);
		//if (times == 0)
		//	sinta = 1;
		//else
		//	sinta = sinta1;
		...
		//{
		//	X1[i] = X1[i] * sinta;
		//	x[i + startpos[1] + extra_nodes] = x[i + startpos[1] + extra_nodes] + X1[i];
		//}
		//getB(Binc, A1_0, b1_0, X1, M1, N1, startpos[1]);

		//2-9
		for (int kk = 1; kk != N_circle + 1; kk++)
		{
			for (int i = 0; i != M1; i++)
				B1[i] = Binc[i + startpos[kk]];
			cgmethod(A1, b1, B1, X1, M1, N1, N_matrix);
			if (times == 0)
				sinta = 1;
			else
				sinta = sinta1;
			...
			{
				X1[i] = X1[i] * sinta;
				x[i + startpos[kk] + extra_nodes] = x[i + startpos[kk] + extra_nodes] + X1[i];
			}
			getB(Binc, A1, b1, X1, M1, N1, startpos[kk]);
		}

		////最后一组
		//for (int i = 0; i != M1; i++)
		//	B1[i] = Binc[i + startpos[N_circle]];
		//cgmethod(A1_2, b1_2, B1, X1, M1, N1, N_matrix);
		//if (times == 0)
		//	sinta = 1;
		//else
		//	sinta = sinta1;
		...
		//{
		//	X1[i] = X1[i] * sinta;
		//	x[i + startpos[N_circle] + extra_nodes] = x[i + startpos[N_circle] + extra_nodes] + X1[i];
		//}
		//getB(Binc, A1_2, b1_2, X1, M1, N1, startpos[N_circle]);


		//10
		for (int i = 0; i != M2; i++)
			B2[i] = Binc[i + startpos[N_circle + 1]];
		cgmethod(A2, b2, B2, X2, M2, N2, N_matrix);
		if (times == 0)
			sinta = 1;
		else
			sinta = sinta1;
		...
		{
			X2[i] = X2[i] * sinta;
			x[i + startpos[N_circle + 1] + extra_nodes] = x[i + startpos[N_circle + 1] + extra_nodes] + X2[i];
		}
		getB(Binc, A2, b2, X2, M2, N2, startpos[N_circle + 1]);
